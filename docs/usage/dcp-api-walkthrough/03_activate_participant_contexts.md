...
```bash
curl -X POST "${ISSUER_IDENTITY}/v1beta/participants/${ISSUER_CONTEXT}/state?isActive=true" \
...
```bash
curl -X POST "${IDH_IDENTITY}/v1beta/participants/${IDH_CONTEXT}/state?isActive=true" \
...
curl -s "${ISSUER_IDENTITY}/v1beta/participants/${ISSUER_CONTEXT}" \
  -H "x-api-key: ${ISSUER_ADMIN_KEY}" | jq .state

# Check Holder participant state
curl -s "${IDH_IDENTITY}/v1beta/participants/${IDH_CONTEXT}" \
...
```bash
curl -X POST "${ISSUER_IDENTITY}/v1beta/participants/${ISSUER_CONTEXT}/state?isActive=false" \
  -H "x-api-key: ${ISSUER_ADMIN_KEY}"
```
