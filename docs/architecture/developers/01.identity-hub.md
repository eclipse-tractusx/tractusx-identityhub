...
1. On the holder side, the holder manages credentials and uses them in verifiable presentations.
...
- **DID Service (DIDS)**: Management publication of DID Documents (e.g. `did:web`).
...
3. **DID Management**:
    - The DID Service creates and publishes DID Documents using `did:web`.
4. **Token Issuance**:
